...
pub struct ServerState {
    ...
    /// 认证管理器 / Auth manager
    pub auth: Arc<crate::core::auth::AuthManager>,
}
...
async fn login_handler(AxumState(s): AxumState<ServerState>, Json(body): Json<LoginBody>) -> ApiResult<serde_json::Value> {
    match s.auth.login(&body.username, &body.password) {
        Some(token) => Ok(Json(serde_json::json!({ "token": token, "username": body.username }))),
        None => Err(ApiError("用户名或密码错误".to_string())),
    }
}

async fn auth_check_handler(AxumState(s): AxumState<ServerState>) -> ApiResult<serde_json::Value> {
    Ok(Json(serde_json::json!({ "valid": true, "username": s.auth.username() })))
}

#[derive(Deserialize)]
struct ChangePasswordBody { old_password: String, new_username: String, new_password: String }

async fn change_credentials_handler(AxumState(s): AxumState<ServerState>, Json(body): Json<ChangePasswordBody>) -> ApiResult<serde_json::Value> {
    let cur = s.auth.username();
    if !s.auth.verify(&cur, &body.old_password) { return Err(ApiError("旧密码错误".to_string())); }
    if body.new_username.trim().is_empty() || body.new_password.is_empty() { return Err(ApiError("用户名和密码不能为空".to_string())); }
    s.auth.change_credentials(&body.new_username, &body.new_password).map_err(ApiError::from)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}

#[derive(Deserialize)]
struct SseTokenQuery { token: Option<String> }

async fn auth_middleware(AxumState(s): AxumState<ServerState>, req: axum::http::Request<axum::body::Body>, next: axum::middleware::Next) -> std::result::Result<axum::response::Response, StatusCode> {
    let path = req.uri().path();
    if path == "/api/login" || path == "/api/events" { return Ok(next.run(req).await); }
    let token = req.headers().get(axum::http::header::AUTHORIZATION).and_then(|v| v.to_str().ok()).and_then(|s| s.strip_prefix("Bearer ")).map(|s| s.to_string());
    match token {
        Some(t) if s.auth.check_token(&t) => Ok(next.run(req).await),
...
pub fn build_router(state: ServerState) -> Router {
    ...
    let main_router: Router<()> = Router::new()
        ...
        .route("/api/auth/change", post(change_credentials_handler))
...
) -> std::result::Result<Sse<impl tokio_stream::Stream<Item = Result<sse::Event, Infallible>>>, StatusCode> {
    match &q.token {
        Some(t) if s.auth.check_token(t) => {}
...
pub async fn run_server(port: u16) {
    ...
    let server_state = ServerState {
        ...
        auth: crate::core::auth::AuthManager::new(),
    };
