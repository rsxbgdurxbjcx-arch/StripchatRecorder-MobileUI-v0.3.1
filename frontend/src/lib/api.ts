...
// ─── 认证 token 管理 / Auth token management ─────────────────────────────────

const TOKEN_KEY = "sr_auth_token";
const USERNAME_KEY = "sr_auth_username";
...
function ensureSse() {
        ...
        const connect = () => {
                // SSE 通过查询参数传递 token（EventSource 不支持自定义 header）
...
/**
 * 检查当前 token 是否有效 / Check if current token is valid.
 */
