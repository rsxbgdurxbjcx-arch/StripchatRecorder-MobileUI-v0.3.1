...
<script setup lang="ts">
	...
	import { changeCredentials, clearAuth, getUsername } from "@/lib/api";
	...
	const cpNewUsername = ref("");
	const cpNewPassword = ref("");
	const cpLoading = ref(false);

	async function handleChangeCredentials() {
		if (!cpOldPassword.value || !cpNewUsername.value || !cpNewPassword.value) {
			toast(t("changePassword.wrongOldPassword"), "error");
			return;
		}
		cpLoading.value = true;
		try {
			const result = await changeCredentials(cpOldPassword.value, cpNewUsername.value, cpNewPassword.value);
			if (result.ok) {
				toast(t("changePassword.success"), "success");
				cpOldPassword.value = "";
				cpNewUsername.value = "";
				cpNewPassword.value = "";
				setTimeout(() => { router.push("/login"); window.location.reload(); }, 1500);
			} else {
				toast(result.error || t("changePassword.failed"), "error");
			}
		} catch (e) {
...
<template>
	<div class="flex flex-col gap-5 max-w-160">
		...
		<form v-else class="flex flex-col gap-7">
			...
			<section class="space-y-3 border-t pt-4 pb-12">
				<h3 class="text-sm font-semibold">{{ t("changePassword.title") }}</h3>
				<div class="space-y-2">
					<Label>{{ t("changePassword.oldPassword") }}</Label>
					<Input v-model="cpOldPassword" type="password" :placeholder="t('changePassword.oldPasswordPlaceholder')" autocomplete="current-password" />
				</div>
				<div class="space-y-2">
					<Label>{{ t("changePassword.newUsername") }}</Label>
					<Input v-model="cpNewUsername" type="text" :placeholder="t('changePassword.newUsernamePlaceholder')" autocomplete="username" />
				</div>
				<div class="space-y-2">
					<Label>{{ t("changePassword.newPassword") }}</Label>
					<Input v-model="cpNewPassword" type="password" :placeholder="t('changePassword.newPasswordPlaceholder')" autocomplete="new-password" />
				</div>
				<div class="flex gap-2">
					<Button type="button" :disabled="cpLoading" @click="handleChangeCredentials">
						{{ cpLoading ? t("common.loading") : t("changePassword.submit") }}
					</Button>
