...
<script setup lang="ts">
	...
	import { Loader2, Image, Play, Wrench, Trash2 } from "@lucide/vue";
	...
	/** 是否为移动端窄屏（< 768px），用于表格紧凑渲染与滚动容器切换 / Whether mobile narrow screen */
	...
	/**
	 * 格式化录制开始时间。移动端使用短格式（MM-DD HH:mm）以压缩列宽。
	 * Format recording start time. Mobile uses short format (MM-DD HH:mm) to shrink column width.
	 */
	function fmtStartTime(s: string): string {
		const d = new Date(s);
		if (!isMobile.value) return d.toLocaleString();
...
<template>
	<div class="flex flex-col h-full min-h-0 md:h-auto">
		...
		<header
			ref="headerEl"
			class="flex items-start justify-between gap-2 md:gap-4 shrink-0 pb-3 md:pb-4 bg-background md:sticky md:top-0 z-20"
		>
			<div class="flex-1 min-w-0">
				<h1 class="text-xl font-bold mb-0.5">{{ t("recordings.title") }}</h1>
				<div
					class="flex items-center gap-2 md:gap-3 text-xs md:text-sm text-muted-foreground flex-wrap"
				>
					<span>{{
			...
			<div class="flex gap-1.5 md:gap-2 shrink-0 flex-wrap justify-end">
				<Button
			...
			<Table v-else class="recordings-table">
				...
				>
					<TableRow>
						...
						<TableHead class="min-w-45 rec-pp">{{
						...
						<template v-if="!collapsedGroups.has(group.username)">
							<TableRow v-for="f in group.files" :key="f.path" class="relative">
								<template v-if="f.status === 'merging_waiting' || f.status === 'merging'">
									...
									<TableCell class="font-medium w-px whitespace-nowrap pl-7 rec-filename">
								...
								<template v-else>
									...
									<TableCell class="min-w-45 rec-pp">
									...
									<TableCell>
										<div class="flex gap-1 md:gap-1.5">
											<Button
												...
												><span class="rec-btn-text">{{ t("recordings.actions.play") }}</span
												><Play class="rec-btn-icon size-3" /></Button
											>
											<Button
											...
											>
												...
												/>
												<template v-else>
													<span class="rec-btn-text">{{
														t("recordings.actions.postprocess")
													}}</span>
													<Wrench class="rec-btn-icon size-3" />
												</template>
											</Button>
											<Button
												size="sm"
												...
												"
												@click="deleteFile(f)"
												><span class="rec-btn-text">{{ t("recordings.actions.delete") }}</span
												><Trash2 class="rec-btn-icon size-3" /></Button
											>
										</div>
									</TableCell>
								</template>
